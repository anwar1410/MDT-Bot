...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'my_identity_select') {
        ...
        if (interaction.values[0] === 'my_info') {
            ...
            // توليد صورة ديناميكية باستخدام Jimp بدلاً من Canvas
...
try {
  ...
  // قناع دائري
  const mask = new Jimp(120, 120, 0x00000000);
  for (let x = 0; x < 120; x++) {
    for (let y = 0; y < 120; y++) {
      const d = Math.sqrt((x - 60) ** 2 + (y - 60) ** 2);
      if (d <= 60) mask.setPixelColor(0xFFFFFFFF, x, y);
    }
  }
...
// معلومات على اليمين
image.print(fontMed, 400, 100, '**** - الرقم');
image.print(
  fontMed,
  400,
  140,
  `تاريخ الميلاد: ${identity.day.padStart(2, '0')}/${convertArabicMonthToNumber(identity.month)}/${identity.year}`
);
image.print(fontMed, 400, 180, `المدينة: ${identity.city}`);
...
async function createViolationPage(identity, userViolations, pageNumber, interaction) {
    ...
    // إنشاء صورة Jimp
    const image = new Jimp(700, 430, '#888888');
    
    // تحميل الخطوط
    const titleFont = await Jimp.loadFont(Jimp.FONT_SANS_32_BLACK);
    const subtitleFont = await Jimp.loadFont(Jimp.FONT_SANS_24_BLACK);
    const textFont = await Jimp.loadFont(Jimp.FONT_SANS_16_BLACK);
    const boldFont = await Jimp.loadFont(Jimp.FONT_SANS_16_BLACK);
    
    // عنوان "أبشر" باللون الأخضر
    const abshirColor = Jimp.rgbaToInt(23, 156, 75, 255); // #179c4b
    image.print(titleFont, 10, 10, 'أبشر');
    
    // عنوان "مخالفاتي"
    image.print(subtitleFont, 10, 50, 'مخالفاتي');
    
    // مربع الاسم الأسود
    const nameBox = new Jimp(220, 38, '#111222');
    image.composite(nameBox, 220, 30);
    
    // اسم المستخدم في المربع
    image.print(boldFont, 220, 35, identity.fullName);
    
    // دائرة بيضاء للصورة الشخصية
    const circleMask = new Jimp(80, 80, 0x00000000);
    for (let x = 0; x < 80; x++) {
        for (let y = 0; y < 80; y++) {
            const distance = Math.sqrt((x - 40) ** 2 + (y - 40) ** 2);
            if (distance <= 38) {
    ...
    for (let i = 0; i < pageViolations.length; i++) {
        ...
        const violationColor = v.executed ? '#27ae60' : '#e74c3c';
        const violationBox = new Jimp(580, 38, violationColor);
        image.composite(violationBox, 60, y);
        
        const violationText = `${v.title} - ${v.desc} - $${v.fine} - ${v.executed ? 'مسددة' : 'غير مسددة'}`;
        image.print(textFont, 70, y + 10, violationText);
        y += 45;
    }
...
async function createCrimePage(foundIdentity, userCrimes, pageNumber, interaction, noCrimes = false) {
    ...
    // إنشاء صورة Jimp
    const image = new Jimp(900, 480, '#181818');
    ...
    try {
        const officerAvatarImg = await Jimp.read(officerAvatar);
        officerAvatarImg.resize(76, 76);
        officerAvatarImg.mask(circleMask, 0, 0);
        image.composite(officerAvatarImg, 20, 20);
    } catch (error) {
        console.error('خطأ في تحميل صورة العسكري:', error);
    }
    
    // اسم العسكري بجانب صورته
    image.print(boldFont, 120, 50, officerName);
    ...
    try {
        ...
        userAvatar.resize(76, 76);
        userAvatar.mask(circleMask, 0, 0);
        image.composite(userAvatar, 800, 20);
    } catch (error) {
        console.error('خطأ في تحميل صورة المستخدم:', error);
    }
    
    // اسم الشخص المطلوب بجانب صورته
    image.print(boldFont, 700, 50, foundIdentity.fullName);
    
    // عنوان "سجل الجرائم"
    image.print(titleFont, 350, 120, 'سجل الجرائم');
    
    // إذا لم توجد جرائم، عرض نص أحمر
    if (noCrimes) {
        image.print(subtitleFont, 300, 220, 'لا يوجد جرائم لهذا الشخص');
    }
    ...
    if (!noCrimes) {
        ...
        for (const crime of pageCrimes) {
            ...
            const crimeBox = new Jimp(200, 80, boxColor);
            
            // إضافة حدود للمربع
            const borderColor = 0xFFFFFFFF; // أبيض
            for (let i = 0; i < 2; i++) {
                crimeBox.setPixelColor(borderColor, i, 0);
                crimeBox.setPixelColor(borderColor, i, 1);
                crimeBox.setPixelColor(borderColor, 199 - i, 0);
                crimeBox.setPixelColor(borderColor, 199 - i, 1);
                crimeBox.setPixelColor(borderColor, 0, i);
                crimeBox.setPixelColor(borderColor, 1, i);
                crimeBox.setPixelColor(borderColor, 0, 79 - i);
                crimeBox.setPixelColor(borderColor, 1, 79 - i);
            }
            
            image.composite(crimeBox, x, y);
            
            // النصوص داخل المربع
            image.print(boldFont, x + 10, y + 5, crime.title);
            
            // معالجة وصف الجريمة ليتناسب مع المربع
            const desc = crime.desc;
            const maxLength = 25; // عدد الأحرف في السطر الواحد
            
            if (desc.length > maxLength) {
                const firstLine = desc.substring(0, maxLength);
                const secondLine = desc.substring(maxLength, maxLength * 2);
                image.print(smallFont, x + 10, y + 25, firstLine);
                if (secondLine.length > 0) {
                    image.print(smallFont, x + 10, y + 37, secondLine + (desc.length > maxLength * 2 ? '...' : ''));
                }
            } else {
                image.print(smallFont, x + 10, y + 25, desc);
            }
            
            image.print(smallFont, x + 10, y + 55, `${crime.months} MONTHS | $${crime.fine}`);
        ...
        image.print(boldFont, 400, 450, `الصفحة ${pageNumber + 1} من ${Math.ceil(userCrimes.length / crimesPerPage)}`);
    }
